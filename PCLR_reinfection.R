# Requires: reinfection_sample_soma.rds, reinfection_proteins.rds, Reinfection_PCLR_min/,
# and `Reinfection_PCLR_1SE/`.

# Run PCLR on 1 random bootstrap resample of reinfection_sample_soma.rds dataset.
# The results after multiple repetitions are to be compiled in two directories: 
# `Reinfection_PCLR_min` and `Reinfection_PCLR_1SE`. 
...
sample_soma <- readRDS("./reinfection_sample_soma.rds")
...
write.csv(out$nzcs.min, paste0("./Reinfection_PCLR_min/", toString(ii), ".csv"), row.names=F)
write.csv(out$nzcs.1se, paste0("./Reinfection_PCLR_1SE/", toString(ii), ".csv"), row.names=F)
